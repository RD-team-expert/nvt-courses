...
if (!$session) {
    echo "❌ Session 4461 not found!\n";
    exit;
}

echo "🔍 SESSION 4461 ANALYSIS:\n";
echo str_repeat("-", 30) . "\n";
...
if ($session->course_online_id) {
    echo "🧮 ATTENTION SCORE CALCULATION:\n";
    echo str_repeat("-", 35) . "\n";
    ...
    try {
        ...
        $improvement = $calculatedScore - ($session->attention_score ?? 0);
...
// Compare with session 4466
echo "📈 COMPARISON WITH SESSION 4466:\n";
echo str_repeat("-", 35) . "\n";

$session4466 = DB::table('learning_sessions')->where('id', 4466)->first();
if ($session4466) {
    echo "Session 4461 vs 4466:\n";
    echo "├─ Active Playback: " . ($session->active_playback_time ?? 'NULL') . "s vs " . ($session4466->active_playback_time ?? 'NULL') . "s\n";
    echo "├─ Completion: " . ($session->video_completion_percentage ?? 'NULL') . "% vs " . ($session4466->video_completion_percentage ?? 'NULL') . "%\n";
    echo "├─ Heartbeat: " . ($session->last_heartbeat ? 'YES' : 'NO') . " vs " . ($session4466->last_heartbeat ? 'YES' : 'NO') . "\n";
    echo "└─ Attention Score: " . ($session->attention_score ?? 'NULL') . " vs " . ($session4466->attention_score ?? 'NULL') . "\n\n";
}
...
if (($session->attention_score ?? 0) > 0) {
    $improvements[] = "✅ Attention score is calculated";
} else {
    $issues[] = "❌ Attention score is still 0";
}
...
if ($successRate >= 80) {
    echo "🎉 EXCELLENT! The system is working very well!\n";
...
echo "\n✅ Session 4461 analysis completed!\n";