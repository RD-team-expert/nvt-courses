...
- **Traditional Course**: A course managed through the Course model with CourseRegistration tracking
...
- **Compliance Status**: A status indicator (Compliant/At Risk/Non-Compliant) based on deadline adherence and progress
- **Assignment Deadline**: The deadline field in CourseAssignment or CourseOnlineAssignment tables
- **Days Overdue**: The number of days past the assignment deadline
- **Excel Export**: An Excel file (.xlsx) with multiple worksheets
- **Completion Status**: The current state of a course assignment (Completed/In Progress/Overdue)
...
2. WHEN displaying course assignments THEN the System SHALL show employee name, department, course type, course name, completion status, days overdue, progress percentage, completion date, learning score, score band, and compliance status
...
5. WHEN calculating days overdue THEN the System SHALL compute the difference between current date and assignment deadline for incomplete assignments
...
2. WHEN generating the completed courses sheet THEN the System SHALL include only assignments with status "completed"
3. WHEN generating the non-completed courses sheet THEN the System SHALL include assignments with status "in_progress", "assigned", or "overdue"
4. WHEN populating worksheet columns THEN the System SHALL include: Employee Name, Department, Course type, Course Name, Completion Status, DaysOverdue, progress%, Completion Date, Overall Learning Score (0-100), Score Band, and Compliance Status
5. WHEN the export is complete THEN the System SHALL download the file with a timestamped filename in format "user_course_progress_YYYY-MM-DD_HH-mm-ss.xlsx"

### Requirement 4

**User Story:** As an administrator, I want the system to calculate compliance status based on deadlines and progress, so that I can identify at-risk learners.

#### Acceptance Criteria

1. WHEN an assignment has no deadline THEN the System SHALL set compliance status to "Compliant"
2. WHEN an assignment is completed before the deadline THEN the System SHALL set compliance status to "Compliant"
3. WHEN an assignment is incomplete and within 7 days of deadline THEN the System SHALL set compliance status to "At Risk"
4. WHEN an assignment is incomplete and past the deadline THEN the System SHALL set compliance status to "Non-Compliant"
5. WHEN calculating compliance status THEN the System SHALL consider both assignment deadline and current progress percentage
...
1. WHEN retrieving traditional course data THEN the System SHALL query CourseAssignment and CourseRegistration tables
2. WHEN retrieving online course data THEN the System SHALL query CourseOnlineAssignment, LearningSession, and UserContentProgress tables
3. WHEN calculating attention scores for online courses THEN the System SHALL use the simulated attention calculation from CourseOnlineReportController
4. WHEN calculating quiz scores THEN the System SHALL aggregate both regular quiz attempts and module quiz results
5. WHEN displaying course type THEN the System SHALL clearly indicate whether each assignment is "traditional" or "online"
